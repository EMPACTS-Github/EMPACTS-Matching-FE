...
const SDGGoalSection: React.FC<SDGGoalSectionProps> = ({ selectedGoal, onGoalChange }) => {
  const isValidGoal = sdgGoals.some((goal) => goal.value === selectedGoal);

  return (
    <div className="flex flex-col w-full gap-2">
      <label className="text-[14px] font-semibold text-[#09090b]">SDG Goal</label>
      <Select
        ...
        {sdgGoals.map((goal) => (
          <SelectItem
            ...
            className={`text-[14px] font-semibold ${
              selectedGoal === goal.value ? 'text-empacts' : 'text-gray-700 hover:text-gray-900'
            }`}
          >
            {goal.label}
          </SelectItem>
