...
const MentorProfileInfoSubCard: React.FC<MentorProfileInfoSubCardProps> = ({
  countMatches,
  mentorProfile,
    ...
    <Card className="bg-white min-w-lg shadow-lg rounded-lg px-4 py-2">
      <CardHeader className="flex flex-col gap-3 items-center">
        ...
        <div className="flex flex-col justify-center items-center">
          ...
          <p
            className={`text-sm font-semibold ${mentorProfile.status === 'ACTIVATED' ? 'text-success' : 'text-gray-500'}`}
          >
            {mentorProfile.status === 'ACTIVATED'
              ? 'Active'
              : mentorProfile.status === 'ARCHIVED'
                ? 'Archived'
                : 'Deactivated'}
          </p>
        </div>
