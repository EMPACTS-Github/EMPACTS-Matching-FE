...
const DocumentBody: React.FC<DocumentBodyProps> = ({ files, selectedFile, onSelectFile, onDeleteAttachment, onUploadNewFile }) => {
  const fileIcons = {
    docx: DocsIcon,
    csv: SheetsIcon,
    pptx: SlidesIcon,
  ...
  const { isOpen, onOpen: onOpenDeleteDocumentModal, onOpenChange: onOpenChangeDeleteDocumentModal } = useDisclosure();
              ...
              {files.map((file, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <button
                      type="button"
                      onClick={() => handleDocumentDownload(file.attachmentUrl, file.attachmentTitle)}
                    ...
                    >
                      <div className="text-lg text-black font-bold">{getFileName(file.attachmentTitle)}</div>
                      <div className="text-xs text-gray-500">{file.type}</div>
                    </button>
                  </div>
                  <Dropdown placement="bottom-end">
                    <DropdownTrigger>
                      <Button isIconOnly className="bg-transparent border-none p-0" onPress={() => { onSelectFile(file) }}>
                        <MoreHorizontalIcon />
                      </Button>
                    </DropdownTrigger>
                    <DropdownMenu>
                      <DropdownItem key="delete-document" variant="light" className="p-0 bg-transparent">
                        <Button
